...
```
git clone https://github.com/yli147/test_optee.git -b dev-rpxy-optee-smp
cd test_optee
export WORKDIR=`pwd`
```
...
```
cd $WORKDIR
git clone https://github.com/yli147/opensbi.git -b dev-rpxy-optee-smp
cd opensbi
...
```
cd $WORKDIR
git clone https://github.com/yli147/optee_os.git -b dev-rpxy-optee-smp
cd optee_os
make CFG_TEE_CORE_LOG_LEVEL=3 CROSS_COMPILE64=/opt/riscv/bin/riscv64-unknown-linux-gnu- ARCH=riscv CFG_TEE_CORE_NB_CORE=2 CFG_DT=n CFG_RPMB_FS=y CFG_RPMB_WRITE_KEY=y CFG_RV64_core=y CFG_TDDRAM_START=0xF0C00000 CFG_TDDRAM_SIZE=0x800000 CFG_SHMEM_START=0xF1600000 CFG_SHMEM_SIZE=0x200000 PLATFORM=virt ta-targets=ta_rv64 MARCH=rv64imafdc MABI=lp64d
